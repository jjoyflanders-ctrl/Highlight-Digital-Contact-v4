...
function renderQr(el, url, size) {
  ...
  // If something already exists, don't overwrite it (prevents double-renders)
  if (el.children.length > 0) return;

  // Always render a fallback QR first so the space never goes blank
  const fallbackImg = document.createElement("img");
  fallbackImg.alt = "QR Code";
  fallbackImg.width = s;
  fallbackImg.height = s;
  fallbackImg.loading = "lazy";
  fallbackImg.referrerPolicy = "no-referrer";
  fallbackImg.src = `https://quickchart.io/qr?text=${encodeURIComponent(url)}&size=${s}`;
  el.appendChild(fallbackImg);

  // If QRCode.js is available, try to render into a TEMP container first.
  // Only swap in if it succeeds; otherwise keep the fallback image.
